...
class BanditAgent():
    ...
    def choose_action(self):
        ...
        Returns:
            An action index.
        """
        agent_data = {"action_value_estimates":self.action_value_estimates,"action_counts":self.action_counts}
...
class ConstantstepAgent(BanditAgent): # Note the inheritance
    """
    A basic constant step lerning rate bandit learner.

    """

    def __init__(self,policy, bandit_env, initial_estimates=0,stepsize = 0.1):
        self.policy = policy
        self.bandit_env = bandit_env
        self.initial_estimates = initial_estimates # for easy reuse
        self.action_value_estimates = self.initial_estimates * \
    ...
    def update_action_value_estimates(self, action, reward):
        ...
        """
        self.action_counts[action] += 1
        self.action_value_estimates[action] += self.stepsize* \
                                               (reward - self.action_value_estimates[action])
